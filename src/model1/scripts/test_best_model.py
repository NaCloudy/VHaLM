...
Usage: python test_best_model.py --experiment_dir outputs/experiment_name [--save_dir ...] [--ablation_mode ...]
"""
...
def test_model(experiment_dir: str,
               data_config: dict,
               max_samples: int = None,
               save_dir: str = None,
               ablation_mode: str = "multi"):
    ...
    normalized_mode = (ablation_mode or "multi").lower()
    if normalized_mode in {"zero", "zerofill"}:
        normalized_mode = "zero-fill"
    elif normalized_mode not in {"multi", "single", "zero-fill"}:
        print(f"[Warning] Unsupported ablation_mode '{ablation_mode}', fallback to 'multi'.")
        normalized_mode = "multi"
    print(f"Ablation mode: {normalized_mode}")
    ...
    test_metrics, test_samples = evaluator.evaluate_dataset(model=model,
    ...
    if save_dir:
        ...
        preds = [sample['prediction'] for sample in test_samples]
        refs = [sample['reference'] for sample in test_samples]

        preds_path = os.path.join(resolved_save_dir, 'preds.json')
        refs_path = os.path.join(resolved_save_dir, 'refs.json')
        metrics_path = os.path.join(resolved_save_dir, 'metrics.json')
        ...
        filtered_metrics = {k: float(v) for k, v in test_metrics.items() if k != 'CIDEr'}
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default=None, help='Directory to save predictions/references for downstream analysis')
    parser.add_argument('--ablation_mode', type=str, default='multi', help='Ablation mode: multi (default) / single / zero-fill')
    ...
    # Run test
    try:
        results = test_model(experiment_dir,
                     data_config,
