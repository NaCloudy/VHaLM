...
class CaptionEvaluator:
    ...
    @staticmethod
    def _apply_ablation_mode(features: torch.Tensor, mode: str) -> torch.Tensor:
        mode_lc = (mode or "multi").lower()
        if mode_lc == "single":
            num_views = features.size(1)
            center_idx = min(num_views - 1, num_views // 2)
            center = features[:, center_idx:center_idx + 1, :]
            return center.repeat(1, num_views, 1)
        if mode_lc in {"zero", "zero-fill", "zerofill"}:
            num_views = features.size(1)
            center_idx = min(num_views - 1, num_views // 2)
            kept = features[:, center_idx:center_idx + 1, :]
            zeros = torch.zeros_like(features)
            zeros[:, center_idx, :] = kept[:, 0, :]
            return zeros
        return features
    ...
    def evaluate_dataset(self,
                         ...
                         samples_path: str = "outputs/samples/evaluation_results.json",
                         max_samples: int = None,
                         ablation_mode: str = "multi") -> Tuple[Dict[str, float], List[Dict[str, Any]]]:
        """
        ...
        applied_mode = (ablation_mode or "multi").lower()
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(dataloader):
                ...
                if applied_mode != "multi":
                    image_features = self._apply_ablation_mode(image_features, applied_mode)
