...
class ImageFeatureAggregator(nn.Module):
    ...
    def forward(self, image_features: torch.Tensor) -> torch.Tensor:
        ...
        if self.aggregation_method == "mean":
            ...
            query = self.query(mean_features)  # (batch_size, hidden_dim)
            keys = self.key(image_features)  # (batch_size, num_images, hidden_dim)
            values = self.value(image_features)  # (batch_size, num_images, hidden_dim)

            scores = torch.matmul(keys, query.unsqueeze(-1)).squeeze(-1) / self.scale
            weights = torch.softmax(scores, dim=1)
            aggregated = torch.sum(weights.unsqueeze(-1) * values, dim=1)
